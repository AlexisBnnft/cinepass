...
export function MovieCard({ movie, isFavorite, onToggleFavorite, onClick }: MovieCardProps) {
  ...
  return (
    <div
      onClick={onClick}
      className="relative bg-gray-900 border border-gray-800 rounded-xl overflow-hidden hover:border-gray-700 transition-colors cursor-pointer group"
    >
      {/* Favorite button */}
      <button
        onClick={(e) => {
      ...
      <div className="flex gap-4 p-4">
        {/* Poster */}
        <div className="w-20 h-28 rounded-lg overflow-hidden bg-gray-800 shrink-0">
          {movie.poster_url ? (
            <img
        ...
        <div className="flex-1 min-w-0">
          <h3 className="font-semibold text-white group-hover:text-indigo-400 transition-colors truncate pr-8">
            {movie.title}
          </h3>
      ...
      <div className="px-4 pb-3 space-y-1.5">
        {movie.cinemas.slice(0, 3).map((cinema) => (
          <div key={cinema.cinema_name} className="flex items-start gap-2">
            <span className="text-[10px] text-gray-500 w-28 shrink-0 truncate pt-0.5">
            ...
            <div className="flex gap-1 flex-wrap">
              {cinema.showtimes.slice(0, 5).map((st, i) => (
                <span
                  key={i}
                  className="text-[11px] px-1.5 py-0.5 rounded bg-gray-800 text-gray-300"
                >
                  {st.show_time}
                  {st.version !== "VF" && (
                    <span className="text-[9px] text-indigo-400 ml-0.5">{st.version}</span>
                  )}
                </span>
              ))}
              {cinema.showtimes.length > 5 && (
                <span className="text-[10px] text-gray-500 pt-0.5">+{cinema.showtimes.length - 5}</span>
              )}
            </div>
          </div>
        ))}
